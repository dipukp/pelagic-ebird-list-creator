...
logger = logging.getLogger("GenerateList")
...
def AddCellDetails(roundedLat, roundedLon, origLat, origLon, curKeyId, myTime, cellList):
    ...
    #
    # A sample output will be as follows. Note how the suffix increases as you reenter the cell after a gap
    #    [11.2-75.8:1] -->     Start: 2014-12-18 07:40:00     End: 2014-12-18 08:19:00   - Actual cell : 11.2-75.8 (new)
    #    [11.1-75.8:1] -->     Start: 2014-12-18 08:20:00     End: 2014-12-18 08:20:00   - Actual cell : 11.1-75.8 (new)
    #    [11.2-75.8:2] -->     Start: 2014-12-18 08:21:00     End: 2014-12-18 08:27:00   - Actual cell : 11.2-75.8 (repeat)
    #    [11.1-75.8:2] -->     Start: 2014-12-18 08:28:00     End: 2014-12-18 08:41:00   - Actual cell : 11.1-75.8 (repeat)
    #    [11.1-75.7:1] -->     Start: 2014-12-18 08:42:00     End: 2014-12-18 08:47:00   - Actual cell : 11.1-75.7 (new)
    ...
    for row in finalCellDataList:
        if(row["Cell Details"] == myKey):
            if(timeofSighting >= row["Start Time"] and timeofSighting <= row["End Time"]):
                if species in row:
                    if not isinstance(count,(int,long,float)):
                        row[species] = count
                        row[species + "-RF"] = 0
                    else:
                        row[species] += int(count)
    ...
    for line in timePoints:
        ...
        if(timeOfSighting == timePoint):
            ...
            while(inserted == 0):
                ...
                logger.debug("Got the timepoint for the current sighting: {}, {}, {}, {}, {}".format(timeOfSighting, timePoint, species, count, centrePoint))
                keyId += 1

if __name__ == "__main__":
    if (len(sys.argv) < 4 or len(sys.argv) > 5) or (len(sys.argv) == 5 and sys.argv[4] != "debug"):
        print("\nUsage:\n\tGenerateList \"<path to logger file>\" \"<path to data sheet xls>\" \"<path of the output file - csv>\" \n")
    ...
    tripDate = datetime.strptime(dataSheet.cell(3,3).value, "%d/%m/%Y")
    ...
    print ("Trip Date        : " + str(tripDate))
    print ("Trip Start Time  : " + str(dataStartTime))
    print ("Trip End Time    : " + str(dataEndTime))
    print "---------------------------------------------------------------"
    tripStartTime = datetime.combine(tripDate.date(), dataStartTime)
    tripEndTime = datetime.combine(tripDate.date(), dataEndTime)
    logger.debug("Timestamps from sheet: {},{},{}".format(tripDate, tripStartTime, tripEndTime))
    ...
    for row in input_file:
        ...
        if dt < tripStartTime or dt > tripEndTime:
            logger.debug("Got a time stamp which is out of range {}".format(dt))
            continue
        if prevRow is not None:
            diff = dt - prevRow["time"]
            if diff.total_seconds() == 0:
                logger.debug("The GPS file has finer resolution (only min expected by the script). So skipping duplicate record {}".format(dt))
                continue
            timeDiff = diff.total_seconds()/60
        if timeDiff > 1:
            # If two time points are more than a minute apart, extrapolate (linear) the GPS points accordingly
    ...
    for cell in sortedCellList:
        ...
        print(str(cell.key) + " -->     Start: " + str(cellData["Start Time"]) + "     End: " + str(cellData["End Time"]))
        prevEndTime = cellData["End Time"]

    print ("---------------------------------------------------------------")    
    print ("Processing of Data Sheet started ...")

    # Now get details from datasheet, read the species list, and for each species, find the right cell and insert the species details into that list
    ...
    for row_index in range(dataSheet.nrows):
        if(dataStarted == 0):        
            ...
            species = dataSheet.cell(row_index,1).value.upper().encode('ascii', 'ignore').decode('ascii')
    ...
    for row in finalCellDataList:
        ...
        if (len(row) == 7 and td.seconds < 600):
            logger.debug("Discarding the list as there are no birds and duration is less than 10 min: {}-{}, {}, {}".format(row["Lat"], row["Lon"], row["Start Time"], row["End Time"]))
            continue
        else:
            columncnt += 1
