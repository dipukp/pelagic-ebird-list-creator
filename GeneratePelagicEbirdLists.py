...
for cell in sortedCellList:
    entry = cell.key
    if (cellList[entry].startTime == prevEndTime):
        cellList[entry].startTime = cellList[entry].startTime + timedelta(minutes=1)
    if (cellList[entry].startTime >= cellList[entry].endTime):
        cellList[entry].endTime = cellList[entry].startTime + timedelta(minutes=1)
    cellData = {}
